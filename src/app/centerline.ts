    ...
    outer: for (let i = 0; i < working.length; i += 1) {
      for (let j = i + 1; j < working.length; j += 1) {
        ...
        for (const [aEnd, bEnd] of candidates) {
          ...
          const left = orientedPath(working[i], aEnd, true);
          const right = orientedPath(working[j], bEnd, false);
          const bridgeNeeded = gap > 0.12;
          working[i] = bridgeNeeded ? [...left, bPoint, ...right.slice(1)] : [...left, ...right.slice(1)];
          working.splice(j, 1);
          merged = true;
          break outer;
        }
...
export async function extractCenterlines(source: string, targetWidthMm: number, stitchLengthMm = 2.5): Promise<CenterlineResult> {
  ...
  const mmPerPixelX = targetWidthMm / rasterWidth;
  const mmPerPixelY = targetHeightMm / rasterHeight;
  const averageMmPerPixel = (mmPerPixelX + mmPerPixelY) / 2;
  const prunedPixelPaths = pruneShortTerminalBranches(rawPixelPaths, averageMmPerPixel);

  const mmPaths = prunedPixelPaths
    .map((pixels) => pixels.map((p) => ({ x: (p.x + 0.5) * mmPerPixelX, y: (p.y + 0.5) * mmPerPixelY })))
    .filter((points) => pathLength(points) >= 1.2)
    .map((points) => simplify(points, 0.16));

  const cleanedMmPaths = mergeNearbyContinuations(mmPaths);
  const vectorPerMmX = vectorWidth / targetWidthMm;
  const vectorPerMmY = vectorHeight / targetHeightMm;
